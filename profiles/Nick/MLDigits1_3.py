...
secondary_res_predicted_value = 0
secondary_res_success_truths = 0
...
for poop_index, poop_value in enumerate(test_idxs):
	...
	secondary_res_predicted_value = -1
	...
	if ((current_cost - predicted_value_cost) < (0.5*predicted_value_cost)):
	...
	for res_i, res_cost in enumerate(res_costs_SI):
			if res_i == predicted_value or res_cost >= res_current_cost:
				pass
			else:
				res_current_cost = res_cost
				secondary_res_predicted_value = res_i 
	...
	print("The Secondary residual predicted cost value is:  %d" % secondary_res_predicted_value)	
	...
	if (secondary_res_predicted_value == truth) or (res_predicted_value == truth):
		secondary_res_success_truths += 1
	else:
...
print("Correct RES Pri|Sec Predictions: %d " % secondary_res_success_truths)
...
print("Ratio of Correct (RES Pri|Sec) to Total:  %.3f" % (1.0* secondary_res_success_truths/iterations))
print("================================")
